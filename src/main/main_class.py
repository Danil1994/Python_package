...
def sorting_order_by(order: List[Driver], desc: bool) -> List[Driver]:
    order.sort(key=lambda x: x.lap_time, reverse=desc)
    return order


def print_ascending(order: List[Driver]) -> None:
    sorted_order = sorting_order_by(order, False)
...
def print_descending(order: List[Driver]) -> None:
    sorted_order = sorting_order_by(order, True)
    list_with_position = _build_position_list(sorted_order)
    for line in list_with_position:
        print(line)
